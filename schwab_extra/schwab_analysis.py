...
class PackageVersionChecker:
    ...
    def check_package_info(
        self, package_name: str, check_updates: bool = False
    ) -> PackageInfo:
        """
    ...
    def get_all_package_info(self, check_updates: bool = False) -> List[PackageInfo]:
        ...
        """
        return [
            self.check_package_info(pkg, check_updates)
...
class TokenAnalyzer:
    ...
    def print_package_analysis(self, check_updates: bool = False) -> None:
            ...
            if check_updates and pkg_info.is_installed:
                if pkg_info.latest_version:
                    if pkg_info.update_available:
                        version_info += (
                            f" → {pkg_info.latest_version} ⚠️  UPDATE AVAILABLE"
                        )
                    else:
        ...
        if check_updates:
            updates_available = [
                pkg for pkg in package_info_list if pkg.update_available
            ]
            if updates_available:
                ...
                installed_packages = [
                    pkg for pkg in package_info_list if pkg.is_installed
                ]
                print(
    ...
    def print_token_analysis(self, analysis: TokenAnalysis) -> None:
        ...
        print(
            f"Token Age: {days} days, {hours} hours, {minutes} minutes, {seconds} seconds"
        )
        ...
        print(
            f"Time Until Expiry: {analysis.time_until_expiry} seconds "
            f"({analysis.time_until_expiry/60:.1f} minutes)"
        )
        print(f"Is Expired: {analysis.is_expired}")
        print(
            f"Should Refresh ({self.refresh_threshold}s margin): {analysis.should_refresh}"
        )

    def analyze_and_print(
        self, token_path: Path, check_updates: bool = False
    ) -> TokenAnalysis:
        """
...
class SchwabAPITester:
    ...
    def create_client(self, token_path: Path):
        ...
        return client_from_token_file(
            api_key=self.api_key, app_secret=self.app_secret, token_path=str(token_path)
        )
...
def main() -> None:
    ...
    try:
        ...
        print(f"❌ Error: {e}")
        print(f"💡 Use schwab-fetch-new-token to create new Token file.")
        ...
        print(
            f"❌ Timeout error: Consider increasing --timeout value (current: {args.timeout}s)"
        )
